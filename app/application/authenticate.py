...
class Authenticate(Interactor[LoginDTO, UserSchema]):
    ...
    async def __call__(self, data: LoginDTO, password_verify: bool = True) -> Optional[UserSchema]:
        ...
        user = UserSchema(id=result.id, username=result.username, registered_at=result.registered_at, disabled=result.disabled)
        if password_verify and data.password is not None and not verify_password(data.password, result.password):
            raise HTTPException(
